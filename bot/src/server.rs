...
pub async fn server() {
    ...
    let api_server = tokio::spawn(async move {
        HttpServer::new(move || {
            ...
            App::new()
                ...
                .route(
                    "/api/auth/login",
                    web::post().to(handlers::web::auth::login),
                )
                .route(
                    "/api/auth/refresh",
    ...
    let crypto_openai_client = client.clone();

    tokio::spawn(async move {
        handlers::market::crypto::start_crypto_loop(crypto_bot, crypto_openai_client).await;
    });
